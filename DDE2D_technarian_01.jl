...
HHa=1.7    # Habitat enveloppe.  Basically max amount of x (bioenergy) that can be cointained by the habitat without running out of space.
HHa2=0.5    # Habitat enveloppe at which population growth stops (Habitat Carring Capacity)
...
Haoptmov(mov,xmov) = (HHa/mov)*(1/(((mov/optmov)^20)+1))
dxmovt(mov,xmov) = 0.20*(Haoptmov(mov,xmov)-xmov) +0.02*(0.25*(xmov-x0mov0))*10.0^(-0.25*(xmov-x0mov0))

dmovt(mov,xmov)= 0.35*(xmov-x0mov0)  +0.07*dxmovt(mov,xmov)*10.0^(-dxmovt(mov,xmov))

dHHat(mov,xmov)= 0.02*(HHa2-xmov*mov)
...
# isoclines color ###############
plt1 = plot(layout=(1,1),size=(600,400),title="DDE2D_technarian, HHa=1.7")
quiver!(xP,yS,quiver=(0.03*uu,0.03*vv), color=:lightgray)
contour!(0.3:0.001:a, 0.1:0.001:b, dmovt, levels=[0], color=:cyan, colorbar = false, w=3)
contour!(0.3:0.001:a, 0.1:0.001:b, dxmovt, levels=[0], color=:orange,colorbar = false, w=3)

plot!(plt1[1],
xlims = (0.3,a),ylims = (0.2,(b-0.01)),xaxis="[mov]",yaxis="[xmov]",xticks = 0:0.1:a,yticks = 0:0.1:(b-0.01),grid= false, show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_1")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_1big")
...
#read(stdin, Char)
savefig("DDE2D_technarian_01_bw_1")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_bw_1big")

#-----------------------------------------------

 function DE2D_Technarian_f(du,u,p,t)
HHa,optmov,x0mov0 = p
#u[1]=mov
#u[2]=xmov

Haoptmov = (HHa/u[1])*(1/(((u[1]/optmov)^20)+1))
du[2] = 0.20*(Haoptmov-u[2]) +0.02*(0.25*(u[2]-x0mov0))*10.0^(-0.25*(u[2]-x0mov0))
du[1]= 0.35*(u[2]-x0mov0)  +0.07*du[2]*10.0^(-du[2])
end

function DE2D_Technarian_g(du,u,p,t)
HHa,optmov,x0mov0 = p
#u[1]=mov
#u[2]=xmov
du[1]= 0.1*0.20*(u[1]+u[2])
du[2]= 0.1*0.06*(u[1]+u[2])
...
u0 = [0.98, 1.0]; #Initial populations
tspan = (0.0,dur);
ODEprob = ODEProblem(DE2D_Technarian_f,u0,tspan,p);
sol = solve(ODEprob);

SDEprob = SDEProblem(DE2D_Technarian_f,DE2D_Technarian_g,u0,tspan,p);
...
CSV.write("technarian01_ODE.csv",sol)
CSV.write("technarian01_SDE.csv",SDEsol)
#CSV.write("technarian01_DDE.csv",DDEsol)
#CSV.write("technarian01_SDDE.csv",SDDEsol)
...
#read(stdin, Char)
savefig("DDE2D_technarian_01_2")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_2big")
...
#read(stdin, Char)
savefig("DDE2D_technarian_01_bw_2")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_bw_2big")
...
plt2 = plot(layout=(1,1),title="DDE2D Technarian,HHa=1.7",size=(600,400))
...
#plot!(plt2[1],DDEsol,vars=(0,1),label="DDE mov")
#plot!(plt2[1],DDEsol,vars=(0,2), label="DDE xmov")

#plot!(plt2[1],SDDEsol,vars=(0,1),label="SDDE mov")
#plot!(plt2[1],SDDEsol,vars=(0,2), label="SDDE xmov",xaxis="Time[Years]",title="DDE2D Technarian",size=(1000,600), show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_3")

plot!(plt2[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_3big")
...
#read(stdin, Char)
savefig("DDE2D_technarian_01_bw_3")

plot!(plt2[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("DDE2D_technarian_01_bw_3big")
